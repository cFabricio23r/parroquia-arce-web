...
describe('ClergySection', () => {
  ...
  it('presenta al párroco con foto, cargo, resumen y enlace específico', () => {
    ...
    expect(screen.getByRole('link', { name: `Conocer la historia de ${priest.name}` }).getAttribute('href')).toBe('/sacerdotes#parroco')
  ...
  it('presenta ambos y enlaza al auxiliar', () => {
    ...
    expect(screen.getByRole('link', { name: `Conocer la historia de ${assistant.name}` }).getAttribute('href')).toBe('/sacerdotes#auxiliar')
  })
