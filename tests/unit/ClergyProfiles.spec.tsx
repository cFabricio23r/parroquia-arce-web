...
describe('ClergyProfiles', () => {
  it('orienta a Contacto cuando no hay información publicada', () => {
    ...
    expect(screen.getByRole('link', { name: 'Contactar con la parroquia' }).getAttribute('href')).toBe('/contacto')
  })
  it('muestra las historias bajo anclas estables y encabezados', () => {
    render(<ClergyProfiles profiles={[priest, assistant]} />)
    expect(screen.getByRole('heading', { name: priest.name, level: 2 }).closest('article')?.id).toBe('parroco')
    expect(screen.getByRole('heading', { name: assistant.name, level: 2 }).closest('article')?.id).toBe('auxiliar')
  ...
  it.each([null, { ...biography, root: { ...biography.root, children: [] } }])('usa el resumen una vez si la historia está vacía', (empty) => {
