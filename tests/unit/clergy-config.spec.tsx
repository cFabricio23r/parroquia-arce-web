...
const group = (position: string) => Clergy.fields.find((f) => 'name' in f && f.name === position) as GroupField
const validate = (data: Record<string, unknown>, originalDoc: Record<string, unknown> = {}) => {
  ...
  return hook({ data, originalDoc, req: req('contenido'), global: Clergy } as Parameters<typeof hook>[0])
}

describe('Clergy', () => {
  it('solo permite editar a contenido y super-admin', async () => {
    for (const role of [undefined, 'comunicaciones', 'contenido', 'super-admin']) {
      expect(await Clergy.access!.update!({ req: req(role) })).toBe(['contenido', 'super-admin'].includes(role ?? ''))
    }
  })
  ...
  it('permite guardar borradores incompletos', async () => {
    expect(await validate({ pastor: { published: false, name: '' } })).toEqual({ pastor: { published: false, name: '' } })
  })
  it.each(['pastor', 'assistant'])('rechaza publicar %s sin nombre o resumen', async (position) => {
    for (const fields of [{ name: ' ', summary: 'Historia' }, { name: 'Nombre', summary: '\n' }]) {
      await expect(async () => validate({ [position]: { ...fields, published: true } })).rejects.toThrow()
    }
  })
  it('valida la combinación con el documento anterior en updates parciales', async () => {
    ...
    expect(await validate({ pastor: { role: 'Párroco' } }, original)).toEqual({ pastor: { role: 'Párroco' } })
  ...
  it('el checkbox empieza sin publicar', () => {
    for (const position of ['pastor', 'assistant']) {
      expect(group(position).fields.find((f) => 'name' in f && f.name === 'published')).toMatchObject({ defaultValue: false })
    }
  })
})
