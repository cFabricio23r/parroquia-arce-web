...
const canReadProfile =
  (position: (typeof positions)[number]): FieldAccess =>
  ({ req, doc }) =>
    ['super-admin', 'contenido'].includes(req.user?.role ?? '') ||
    doc?.[position]?.published === true
...
const validatePublication: GlobalBeforeValidateHook = ({ data, originalDoc }) => {
  ...
  for (const position of positions) {
    ...
    for (const [field, label] of [
      ['name', 'el nombre'],
      ['summary', 'la presentación breve'],
    ] as const) {
      if (typeof profile[field] !== 'string' || !profile[field].trim()) {
        errors.push({
          path: `${position}.${field}`,
          message: `Completá ${label} antes de publicar este perfil.`,
        })
      }
    }
  }
