...
import { hasBiographyContent, type ClergyProfile } from '@/lib/clergy'
  ...
  return (
    <Container>
      {profiles.length ? profiles.map((profile) => {
        ...
        return (
          <article key={profile.id} id={profile.id} className="scroll-mt-24 border-b border-line-soft py-[clamp(48px,6vw,80px)] last:border-b-0">
            <div className={`grid items-start gap-8 md:gap-14 ${photo ? 'md:grid-cols-[minmax(0,280px)_minmax(0,1fr)]' : ''}`}>
              {photo && (
              ...
              <div className="min-w-0 max-w-[720px]">
                <p className="mb-3 break-words text-[12px] font-bold uppercase tracking-[.12em] text-blue">{profile.role}</p>
                <h2 className="break-words font-display text-[clamp(32px,4vw,46px)] font-medium leading-[1.08]">{profile.name}</h2>
        ...
        <div className="py-[clamp(48px,6vw,80px)]">
          <p className="mb-6 text-[18px] text-muted">Pronto compartiremos información sobre nuestros sacerdotes.</p>
