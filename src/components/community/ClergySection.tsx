...
import { getClergyPhoto, type ClergyProfile } from '@/lib/clergy'
    ...
    <section className="bg-bg-soft py-[clamp(56px,7vw,96px)]">
      <Container>
        <SectionHead
          title="Nuestros"
          emphasis="sacerdotes"
          lead="Conocé a quienes acompañan la vida y la fe de nuestra comunidad."
        />
              ...
              >
                {photo && (
                  <div className="aspect-[4/3] overflow-hidden">
                    <MediaImage cover={photo} className="object-[50%_25%]" />
                  </div>
                )}
                <div className="flex flex-col items-start p-6 sm:p-8">
                  <p className="mb-3 max-w-full break-words text-[12px] font-bold uppercase tracking-[.12em] text-blue">
                    {profile.role}
                  </p>
                  <h3 className="max-w-full break-words font-display text-[clamp(26px,3vw,34px)] font-medium leading-[1.12]">
                    {profile.name}
                  </h3>
                  <p className="mb-6 mt-4 max-w-full whitespace-pre-line break-words leading-[1.65] text-muted">
                    {profile.summary}
                  </p>
                  <Link
